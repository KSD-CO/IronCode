...
2. Every error has a name. Don't say "handle errors." Name the specific error class (NamedError subclass), what triggers it, what catches it, what the user sees, and whether it's tested.
...
* ASCII diagrams in code comments for complex designs — State machines, Service pipelines, Request flows, Module boundaries, Tests (non-obvious setup).
...
git diff dev --stat                            # What's already changed
git stash list                                 # Any stashed work
grep -r "TODO\|FIXME\|HACK\|XXX" --include="*.ts" --include="*.tsx" -l
find . -name "*.ts" -newer bun.lock | head -20  # Recently touched files
```
...
**STOP.** Use the `question` tool once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
* Data flow — all four paths. For every new data flow, ASCII diagram the:
    ...
    * Null/undefined path (input is missing — what happens?)
...
- `packages/ironcode/src/` — core CLI logic, commands, tools, providers
- `packages/ironcode/native/tool/` — Rust FFI native tools
- `packages/app/` — web UI (if applicable)
...
```
  ...
  ExampleService.call()    | API timeout                 | TimeoutError
                           ...
                           | API returns malformed JSON  | JSON.parse SyntaxError
                           | Connection pool exhausted   | ConnectionError
                           | Record not found            | NotFoundError
  -------------------------|-----------------------------|-----------------

  ERROR CLASS                  | CAUGHT?   | CATCH ACTION             | USER SEES
  -----------------------------|-----------|--------------------------|------------------
  TimeoutError                 | Y         | Retry 2x, then throw     | "Service temporarily unavailable"
  RateLimitError               | Y         | Backoff + retry           | Nothing (transparent)
  JSON SyntaxError             | N ← GAP   | —                        | Crash ← BAD
  ConnectionError              | N ← GAP   | —                        | Crash ← BAD
  NotFoundError                | Y         | Return undefined, log     | "Not found" message
```
Rules for this section:
* Catching all errors with a generic `catch (e)` is ALWAYS a smell. Name the specific error types.
...
Use IronCode's `NamedError.create()` pattern for all new error types.
**STOP.** Use the `question` tool once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
* Input validation. For every new user input: is it validated with Zod, sanitized, and rejected loudly on failure? What happens with: null, empty string, string when number expected, string exceeding max length, unicode edge cases, HTML/script injection attempts?
...
* Injection vectors. Command injection via `Bun.spawn`/`Bun.spawnSync`, path traversal, template injection, LLM prompt injection — check all.
...
```
  ...
  ---------------------|------------------------|----------|--------
  CLI command          | Invalid args           | ?        |
                       | Missing required args  | ?        |
                       | Interrupted mid-run    | ?        |
  Async operation      | User cancels (Ctrl+C)  | ?        |
                       | Operation times out    | ?        |
                       | Retry while in-flight  | ?        |
  File operations      | File doesn't exist     | ?        |
                       | No write permission    | ?        |
                       | Disk full              | ?        |
  TUI interaction      | Terminal resized       | ?        |
...
Evaluate:
* Code organization and module structure. Does new code fit existing IronCode namespace patterns? If it deviates, is there a reason?
* DRY violations. Be aggressive. If the same logic exists elsewhere, flag it and reference the file and line.
* Naming quality. Are new classes, methods, and variables named for what they do, not how they do it?
* Error handling patterns. (Cross-reference with Section 2 — this section reviews the patterns; Section 2 maps the specifics.)
* Missing edge cases. List explicitly: "What happens when X is undefined?" "When the API returns 429?" etc.
* Over-engineering check. Any new abstraction solving a problem that doesn't exist yet?
* Under-engineering check. Anything fragile, assuming happy path only, or missing obvious defensive checks?
* Cyclomatic complexity. Flag any new function that branches more than 5 times. Propose a refactor.
**STOP.** Use the `question` tool once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
```
  NEW CLI COMMANDS/FLAGS:
    [list each new user-visible command or flag]
  ...
  NEW INTEGRATIONS / EXTERNAL CALLS:
    [list each — API providers, file system, spawn, etc.]
...
IronCode test patterns: Use `bun test` with `describe`/`test`/`expect`. Mock with `mock.module()`. For Rust FFI, test both the Bun FFI path and the fallback JS path.
**STOP.** Use the `question` tool once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 7: Performance Review
Evaluate:
* Memory usage. For every new data structure: what's the maximum size in production? Watch for unbounded arrays/maps.
* Bun-specific performance. Are we using `Bun.file()` instead of `fs.readFile()`? `Bun.spawn` instead of `child_process`? `Bun.write` instead of `fs.writeFile()`?
* Startup time impact. New imports at module level add to startup time. Lazy import where possible.
* Streaming vs buffering. For large outputs (LLM responses, file reads), are we streaming or buffering the whole thing in memory?
* Connection pool pressure. New HTTP connections, WebSocket connections?
* Rust FFI overhead. Are we crossing the FFI boundary too frequently? Batch operations where possible.
* Hot paths. Top 3 slowest new codepaths and estimated latency.
**STOP.** Use the `question` tool once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 8: Observability & Debuggability Review
New systems break. This section ensures you can see why.
Evaluate:
* Logging. For every new codepath: structured log lines (via `Log.create()`) at entry, exit, and each significant branch?
* Metrics. For every new feature: what metric tells you it's working? What tells you it's broken?
* Tracing. For new cross-service or cross-process flows: trace IDs propagated?
* Debuggability. If a bug is reported 3 weeks post-ship, can you reconstruct what happened from logs alone?
* Error context. Are errors logged with sufficient context (what was being attempted, with what arguments, for what user)?
...
Evaluate:
* Binary distribution. IronCode ships as a compiled Bun binary via npm. Does this change affect the build? Does it need new platform targets?
* Feature flags. Should any part be behind a flag (`Flag` namespace in `flag.ts`)?
* Backward compatibility. Will the new version break existing user configs (`.ironcode/ironcode.jsonc`)?
* Migration path. If config format changes, does the new version handle old configs gracefully?
* Rollback plan. Explicit step-by-step. User can always `npm install ironcode-ai@previous-version`.
* Post-deploy verification. What automated checks should run after publish?
...
Complete table of every method that can fail, every error class, caught status, catch action, user impact.
...
### TODOS.md updates
Present each potential TODO as its own individual question. Never batch TODOs — one per question. Never silently skip this step.
...
Identify at least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each delight opportunity as its own individual question. Never batch them. For each one, describe what it is, why it would delight users, and effort estimate. Then present options: **A)** Add to TODOS.md as a vision item **B)** Skip **C)** Build it now in this PR.
