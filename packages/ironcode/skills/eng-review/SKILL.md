...
* For particularly complex designs or behaviors, embed ASCII diagrams directly in code comments in the appropriate places: Modules (data relationships, state transitions), Request handlers (request flow), Mixins (mixin behavior), Services (processing pipelines), and Tests (what's being set up and why) when the test structure is non-obvious.
...
**STOP.** For each issue found in this section, use the `question` tool individually. One issue per call. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one question. Only proceed to the next section after ALL issues in this section are resolved.

### 2. Code quality review
Evaluate:
* Code organization and module structure. Does new code follow existing IronCode namespace patterns?
* DRY violations—be aggressive here.
* Error handling patterns and missing edge cases (call these out explicitly). Use IronCode's `NamedError.create()` pattern for typed errors.
...
Evaluate:
* Memory usage and unbounded data structures.
* Bun-specific performance — using `Bun.file()`, `Bun.spawn`, `Bun.write` instead of Node.js equivalents?
* Startup time impact — lazy imports where possible.
* Streaming vs buffering for large data.
* Rust FFI crossing frequency — batch where possible.
* Slow or high-complexity code paths.

**STOP.** For each issue found in this section, use the `question` tool individually. One issue per call. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one question. Only proceed to the next section after ALL issues in this section are resolved.

## CRITICAL RULE — How to ask questions
Every question MUST: (1) present 2-3 concrete lettered options, (2) state which option you recommend FIRST, (3) explain in 1-2 sentences WHY that option over the others, mapping to engineering preferences. No batching multiple issues into one question. No yes/no questions. Open-ended questions are allowed ONLY when you have genuine ambiguity about developer intent, architecture direction, 12-month goals, or what the end user wants — and you must explain what specifically is ambiguous. **Exception:** SMALL CHANGE mode intentionally batches one issue per section into a single question at the end — but each issue in that batch still requires its own recommendation + WHY + lettered options.
...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only use the `question` tool when there is a genuine decision with meaningful tradeoffs.
...
### Diagrams
The plan itself should use ASCII diagrams for any non-trivial data flow, state machine, or processing pipeline. Additionally, identify which files in the implementation should get inline ASCII diagram comments — particularly modules with complex state transitions, services with multi-step pipelines, and mixins with non-obvious behavior.
...
* When asking a question, label each option with issue NUMBER and option LETTER so I don't get confused.
