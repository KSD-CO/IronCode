...
2. **Simplify:** Explain the problem in plain English. No raw function names, no jargon.
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/code-ship` from the
...
1. Does the README's feature list match what AGENTS.md describes?
2. Does ARCHITECTURE's component list match CONTRIBUTING's project structure?
3. Does CHANGELOG's latest version match the VERSION file (or package.json version)?
...
5. Flag any contradictions between documents. Auto-fix clear factual inconsistencies. Ask the
...
1. **If VERSION file and package.json version both don't exist:** Skip silently.
...
4. **If version was already bumped:** Check whether the bump covers the full scope of changes:
   ...
   c. If CHANGELOG covers everything: Skip — "VERSION: Already bumped, covers all changes."
...
5. If no PR exists: skip with message "No PR found — skipping body update."
...
- Already bumped — version was set by /code-ship
