...
import { ProviderRegistry } from "../../src/provider/provider"
...
test("plan agent denies edits except .ironcode/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".ironcode/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("custom agent from config creates new agent", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const parsed = ProviderRegistry.parse(custom!.model!)
...
test("custom agent config overrides native agent properties", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const build = await Agent.get("build")
      expect(build).toBeDefined()
      expect(build?.model).toBeDefined()
      const parsed = ProviderRegistry.parse(build!.model!)
      expect(parsed.providerID).toBe("anthropic")
      expect(parsed.modelID).toBe("claude-3")
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".ironcode", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("build")
        const skillDir = path.join(tmp.path, ".ironcode", "skill", "perm-skill")
