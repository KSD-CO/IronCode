...
const managedConfigDir = process.env.IRONCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "ironcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "ironcode.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "ironcode.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "ironcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .ironcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ironcodeDir = path.join(dir, ".ironcode")
      await fs.mkdir(ironcodeDir, { recursive: true })
      const agentDir = path.join(ironcodeDir, "agent")
...
test("loads agents from .ironcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ironcodeDir = path.join(dir, ".ironcode")
      await fs.mkdir(ironcodeDir, { recursive: true })

      const agentsDir = path.join(ironcodeDir, "agents")
...
test("loads commands from .ironcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ironcodeDir = path.join(dir, ".ironcode")
      await fs.mkdir(ironcodeDir, { recursive: true })

      const commandDir = path.join(ironcodeDir, "command")
...
test("loads commands from .ironcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const ironcodeDir = path.join(dir, ".ironcode")
      await fs.mkdir(ironcodeDir, { recursive: true })

      const commandsDir = path.join(ironcodeDir, "commands")
...
test("does not try to install dependencies in read-only IRONCODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.IRONCODE_CONFIG_DIR
  process.env.IRONCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.IRONCODE_CONFIG_DIR
    else process.env.IRONCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable IRONCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "ironcode.json"),
        JSON.stringify({ $schema: "https://ironcode.cloud/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "ironcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .ironcode config
      const projectDir = path.join(dir, "project")
      const ironcodeDir = path.join(projectDir, ".ironcode")
      await fs.mkdir(ironcodeDir, { recursive: true })
      ...
      // Local .ironcode config with different plugins
      await Bun.write(
        path.join(ironcodeDir, "ironcode.json"),
        JSON.stringify({
          $schema: "https://ironcode.cloud/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ironcode config with some overlapping plugins
      await Bun.write(
        path.join(ironcodeDir, "ironcode.json"),
        JSON.stringify({
          $schema: "https://ironcode.cloud/config.json",
...
// Note: preload.ts sets IRONCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .ironcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ironcode directory config enables it
      const ironcodeDir = path.join(dir, ".ironcode")
      await fs.mkdir(ironcodeDir, { recursive: true })
      await Bun.write(
        path.join(ironcodeDir, "ironcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/ironcode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/ironcode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-ironcode@2.4.3")).toBe("oh-my-ironcode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@ironcode/plugin@2.0.0")).toBe("@ironcode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-ironcode@2.4.3", "file:///project/.ironcode/plugin/oh-my-ironcode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.ironcode/plugin/oh-my-ironcode.js")
  })
  ...
  test("local plugin directory overrides global ironcode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const ironcodeDir = path.join(projectDir, ".ironcode")
        const pluginDir = path.join(ironcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("IRONCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["IRONCODE_DISABLE_PROJECT_CONFIG"]
    process.env["IRONCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["IRONCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["IRONCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .ironcode/ directories when flag is set", async () => {
    const originalEnv = process.env["IRONCODE_DISABLE_PROJECT_CONFIG"]
    process.env["IRONCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .ironcode directory with a command
          const ironcodeDir = path.join(dir, ".ironcode", "command")
          await fs.mkdir(ironcodeDir, { recursive: true })
          await Bun.write(path.join(ironcodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .ironcode should NOT be in directories list
          const hasProjectIroncode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectIroncode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["IRONCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["IRONCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["IRONCODE_CONFIG_DIR"]
      process.env["IRONCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["IRONCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["IRONCODE_CONFIG_DIR"]
      } else {
        process.env["IRONCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("IRONCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["IRONCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["IRONCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["IRONCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["IRONCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from IRONCODE_CONFIG_DIR, not project
