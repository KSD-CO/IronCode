...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run ironcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for ironcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/ironcode\.cloud\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found ironcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`ironcode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting ironcode...")
        UI.println()

        // Launch ironcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const ironcodeArgs = sessionId ? ["-s", sessionId] : []
        const ironcodeProcess = spawn("ironcode", ironcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          ironcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`ironcode exited with code ${code}`))
          })
          ironcodeProcess.on("error", reject)
        })
      },
    })
  },
