...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall ironcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall IronCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g ironcode-ai",
      pnpm: "pnpm uninstall -g ironcode-ai",
      bun: "bun remove -g ironcode-ai",
      yarn: "yarn global remove ironcode-ai",
      brew: "brew uninstall ironcode",
      choco: "choco uninstall ironcode",
      scoop: "scoop uninstall ironcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall ironcode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using IronCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# ironcode") || content.includes(".ironcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".ironcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".ironcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".ironcode"))
    ) {
      continue
    }
