...
import type { AssistantMessage, Event, IroncodeClient, SessionMessageResponse } from "@ironcode-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: IroncodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `ironcode auth login` in the terminal",
        name: "Login with ironcode",
        id: "ironcode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // IronCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const ironcodeProvider = providers.find((p) => p.id === "ironcode")
    if (ironcodeProvider) {
      if (ironcodeProvider.models["big-pickle"]) {
        return { providerID: "ironcode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(ironcodeProvider.models))
      if (best) {
        return {
