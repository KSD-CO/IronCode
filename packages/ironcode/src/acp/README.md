...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for ironcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal ironcode sessions
...
ironcode acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via ironcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | ironcode acp
```
...
ACP sessions map cleanly to ironcode's internal session model:
