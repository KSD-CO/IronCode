...
// DISABLED: DurableObject not needed anymore
...
type Env = {
  // DISABLED: SYNC_SERVER Durable Object removed
  // SYNC_SERVER: DurableObjectNamespace<SyncServer>
  Bucket: R2Bucket
  WEB_DOMAIN: string
}

// DISABLED: SyncServer Durable Object for session sharing
// Security concerns:
...
export default new Hono<{ Bindings: Env }>()
  ...
  // DISABLED: Share endpoints - security concerns (no auth, rate limiting, etc)
  ...
  .post("/exchange_github_app_token", async (c) => {
    const EXPECTED_AUDIENCE = "ironcode-github-action"
  ...
  /**
   * Used by the GitHub action to get GitHub installation access token given user PAT token (used when testing `ironcode github run` locally)
   */
  ...
  /**
   * Used by the ironcode CLI to check if the GitHub app is installed
   */
