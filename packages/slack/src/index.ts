...
import { createIroncode, type ToolPart } from "@ironcode-ai/sdk"
...
const ironcode = await createIroncode({
  port: 0,
})
console.log("✅ Ironcode server ready")
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new ironcode session...")
    const { client, server } = ironcode
    ...
    console.log("✅ Created ironcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to ironcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Ironcode response:", JSON.stringify(result, null, 2))
