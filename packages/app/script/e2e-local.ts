...
const ironcodeDir = path.join(repoDir, "packages", "ironcode")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "ironcode-e2e-"))
...
const servermod = await import("../../ironcode/src/server/server")
const inst = await import("../../ironcode/src/project/instance")
const server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
console.log(`ironcode server listening on http://127.0.0.1:${serverPort}`)
