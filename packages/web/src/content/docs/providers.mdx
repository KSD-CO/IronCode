---
title: Providers
description: Using any LLM provider in IronCode.
---

import config from "../../../config.mjs"
export const console = config.console

IronCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your IronCode config.
...
in `~/.local/share/ironcode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your IronCode
config.
...
IronCode Zen is a list of models provided by the IronCode team that have been
tested and verified to work well with IronCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with IronCode Zen.
:::

1. Run the `/connect` command in the TUI, select ironcode, and head to [ironcode.cloud/auth](https://ironcode.cloud/auth).
...
It works like any other provider in IronCode and is completely optional to use.
...
To use Amazon Bedrock with IronCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running ironcode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY ironcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile ironcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX ironcode
   ```
   ...
   For project-specific or persistent configuration, use `ironcode.json`:
...
:::info
Using your Claude Pro/Max subscription in IronCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for ironcode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX ironcode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX ironcode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your ironcode config.
...
to store token in ironcode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
IronCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock IronCode
to only use your own GitLab-hosted instance, add the following to your
`ironcode.json` file. It is also recommended to disable session sharing.
...
More documentation on [ironcode-gitlab-auth](https://www.npmjs.com/package/@gitlab/ironcode-gitlab-auth) homepage.

##### Configuration

Customize through `ironcode.json`:
...
{
  "$schema": "https://ironcode.cloud/config.json",
  "plugin": ["@gitlab/ironcode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with ironcode:
...
To use Google Vertex AI with IronCode:
...
2. Set the required environment variables:
   ...
   Set them while running ironcode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id ironcode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through ironcode, you can always configure it yourself.
...
```jsonc title="~/.config/ironcode/ironcode.jsonc"
{
  "$schema": "https://ironcode.cloud/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [ironcode-helicone-session](https://github.com/H2Shami/ironcode-helicone-session) plugin to automatically log each IronCode conversation as a session in Helicone.

```bash
npm install -g ironcode-helicone-session
```
...
```json title="ironcode.json"
{
  "plugin": ["ironcode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each IronCode conversation listed as a separate session.
...
You can configure ironcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="ironcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://ironcode.cloud/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure ironcode to use local models through LM Studio.

```json title="ironcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://ironcode.cloud/config.json",
  "provider": {
    "lmstudio": {
...
You can configure ironcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for IronCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/ironcode) for details.
:::

```json title="ironcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://ironcode.cloud/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with IronCode:
...
3. Copy the API key for use in IronCode.
...
6. **Important**: Before using cloud models in IronCode, you must pull the model information locally:
...
IronCode Zen is a list of tested and verified models provided by the IronCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>IronCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **IronCode Zen**.

   ```txt
   /connect
   ```

3. Enter your IronCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your ironcode config.
...
5. You can also customize them through your ironcode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' ironcode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group ironcode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Ironcode:
...
You can also customize models through your ironcode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with ironcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in ironcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `ironcode.json` file in your project directory:

   ```json title="ironcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://ironcode.cloud/config.json",
     "provider": {
       "myprovider": {
...
```json title="ironcode.json" {9,11,17-20}
{
  "$schema": "https://ironcode.cloud/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow IronCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `ironcode auth list` to see if the credentials
...
2. For custom providers, check the ironcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your ironcode config.
