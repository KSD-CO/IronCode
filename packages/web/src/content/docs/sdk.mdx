---
title: SDK
description: Type-safe JS client for ironcode server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The ironcode JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control ironcode programmatically.
...
Create an instance of ironcode:
...
You can pass a configuration object to customize behavior. The instance still picks up your `ironcode.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${ironcode.server.url}`)
...
If you already have a running instance of ironcode, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@ironcode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*ironcode" },
})
