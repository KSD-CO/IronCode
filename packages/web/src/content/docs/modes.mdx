...
:::caution
Modes are now configured through the `agent` option in the ironcode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in ironcode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the ironcode config.
...
ironcode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.ironcode/plans/*.md` to detail the plan itself
...
Configure modes in your `ironcode.json` config file:
...
- Global: `~/.config/ironcode/modes/`
- Project: `.ironcode/modes/`

```markdown title="~/.config/ironcode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global ironcode config and the project specific config.
...
Create mode files in `.ironcode/modes/` for project-specific modes or `~/.config/ironcode/modes/` for global modes:

```markdown title=".ironcode/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/ironcode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
