...
You can add external tools to IronCode using the _Model Context Protocol_, or MCP. IronCode supports both local and remote servers.
...
You can define MCP servers in your [IronCode Config](https://ironcode.cloud/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="ironcode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/ironcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
IronCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, IronCode will:
...
If the server requires authentication, IronCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `ironcode mcp auth <server-name>`.
...
```bash
ironcode mcp auth my-oauth-server
```
...
```bash
ironcode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, IronCode will store the tokens securely in `~/.local/share/ironcode/mcp-auth.json`.
...
ironcode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in IronCode, alongside built-in tools. So you can manage them through the IronCode config like any other tool.
...
```json title="ironcode.json" {11, 14-18}
{
  "$schema": "https://ironcode.cloud/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
ironcode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect IronCode to your Sentry account.
