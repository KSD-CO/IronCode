...
There are two types of agents in IronCode; primary agents and subagents.
...
IronCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
IronCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

IronCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `ironcode.json` config file:
...
- Global: `~/.config/ironcode/agents/`
- Per-project: `.ironcode/agents/`

```markdown title="~/.config/ironcode/agents/review.md"
---
...
If no temperature is specified, IronCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global IronCode config and the project specific config.
...
The model ID in your IronCode config uses the format `provider/model-id`. For example, if you're using [IronCode Zen](/docs/zen), you would use `ironcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="ironcode.json" {3-6,9-12}
{
  "$schema": "https://ironcode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="ironcode.json" {3-5,8-10}
{
  "$schema": "https://ironcode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `ironcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/ironcode).
:::
...
```markdown title="~/.config/ironcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/ironcode/agents/security-auditor.md"
---
