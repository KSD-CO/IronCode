...
With IronCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, IronCode uses our own `ironcode` theme.
...
IronCode comes with several built-in themes.
...
- Want IronCode to match their terminal's appearance
...
IronCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/ironcode/themes/*.json` or `$XDG_CONFIG_HOME/ironcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.ironcode/themes/*.json`
4. **Current working directory** - Defined in `./.ironcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/ironcode/themes
vim ~/.config/ironcode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .ironcode/themes
vim .ironcode/themes/my-theme.json
```
...
{
  "$schema": "https://ironcode.cloud/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
