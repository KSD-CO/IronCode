...
Tools allow the LLM to perform actions in your codebase. IronCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in IronCode.
...
:::note
This tool is only available when `IRONCODE_EXPERIMENTAL_LSP_TOOL=true` (or `IRONCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the IronCode provider or when the `IRONCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching IronCode:

```bash
IRONCODE_ENABLE_EXA=1 ironcode
```
