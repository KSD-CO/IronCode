...
:::note
This tool is only available when using the IronCode provider or when the `OPENCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching IronCode:

```bash
OPENCODE_ENABLE_EXA=1 ironcode
```
