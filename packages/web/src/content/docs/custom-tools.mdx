---
title: Custom Tools
description: Create tools the LLM can call in ironcode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside ironcode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.ironcode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/ironcode/tools/`.
...
```ts title=".ironcode/tools/database.ts" {1}
...
```ts title=".ironcode/tools/math.ts"
import { tool } from "@ironcode-ai/plugin"
...
```ts title=".ironcode/tools/project.ts" {8}
import { tool } from "@ironcode-ai/plugin"
...
```python title=".ironcode/tools/add.py"
import sys
...
```ts title=".ironcode/tools/python-add.ts" {10}
import { tool } from "@ironcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".ironcode/tools/add.py")
