---
title: Server
description: Interact with ironcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `ironcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an ironcode client can use.
...
```bash
ironcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `ironcode.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
ironcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `ironcode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `ironcode serve` and `ironcode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password ironcode serve
```
...
When you run `ironcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the ironcode server to interact with ironcode programmatically.
:::

This architecture lets ironcode support multiple clients and allows you to interact with ironcode programmatically.

You can run `ironcode serve` to start a standalone server. If you have the
ironcode TUI running, `ironcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the IronCode [IDE](/docs/ide) plugins.
...
The ironcode server exposes the following APIs.
