...
Set `IRONCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `ironcode`, or set `IRONCODE_SERVER_USERNAME` to override it. This applies to both `ironcode serve` and `ironcode web`.

```bash
IRONCODE_SERVER_PASSWORD=your-password ironcode serve
```
