...
description: What's new in IronCode 1.0.
---

IronCode 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same ironcode server.
...
version. However some older versions of IronCode always grab latest.

To upgrade manually, run

```bash
$ ironcode upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ ironcode upgrade 0.15.31
```
